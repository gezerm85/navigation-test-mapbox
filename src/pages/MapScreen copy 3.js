...
import data from "../assets/data/yol_formatted2";
...
const MapScreen = () => {
  ...
  const calculateRoute = () => {
    ...
    setTimeout(() => {
      const start = turf.point(fakeUserLocation || userLocation);
      const end = turf.centroid(turf.polygon(selectedPolygon));
  
      // Kullanıcıya yakın engelleri filtrele
      const nearbyObstacles = geoJsonData.features.filter((feature) => {
        const obstacle = turf.polygon(feature.geometry.coordinates);
        return turf.booleanIntersects(obstacle, turf.buffer(start, 0.5, { units: "kilometers" }));
      });
  
      // Yakın engellerle tampon bölge hesapla
      const bufferDistance = 0.00005; // 5 cm
      const obstacles = turf.featureCollection(
        nearbyObstacles.map((feature) =>
      ...
      // Kısa yol hesapla, filtrelenmiş engellerle
      ...
      if (shortestPath) {
        ...
        const walkingSpeed = 5; // Ortalama insan yürüyüş hızı: 5 km/saat
        const approxDuration = (length / walkingSpeed) * 60; // Süreyi dakika cinsine çevir
  ...
  return (
    <View style={styles.page}>
      <MapboxGL.MapView style={styles.map}>
        <MapboxGL.Camera
          zoomLevel={19}
          centerCoordinate={fakeUserLocation || userLocation || [0, 0]} // Varsayılan koordinat
          ref={(ref) => {
        ...
        <MapboxGL.ShapeSource
          id="geojson-data"
          shape={data} // GeoJSON formatındaki veriniz
        >
          <MapboxGL.LineLayer
            id="geojson-lines"
            style={{
